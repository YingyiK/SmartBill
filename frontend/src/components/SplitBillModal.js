...
const SplitBillModal = ({ isOpen, onClose, expense, onSuccess }) => {
  ...
  useEffect(() => {
    ...
    // 故意去掉 eslint-plugin-react-hooks 规则检查
    ...
    try {
      ...
      const relevant = contacts.filter((c) => {
        if (!expense.participants?.length)
          return splits.some((s) => s.contact_id === c.id);
      ...
      if (splits.length) {
        const ids = splits
          .filter((s) => relevant.some((c) => c.id === s.contact_id))
          .map((s) => s.contact_id);
        ...
        splits.forEach((s) => {
          if (s.contact_id && relevant.some((c) => c.id === s.contact_id)) {
            calc[s.contact_id] = {
              amount: toNum(s.amount_owed),
              items:
                typeof s.items_detail === 'string'
                  ? JSON.parse(s.items_detail)
                  : s.items_detail || [],
              aaDetails: [],
            };
          }
  ...
  const calculateSplitsFromParticipants = (contacts) => {
    ...
    expense.participants.forEach((p) => {
      ...
      let total = 0;
      const items =
        typeof p.items === 'string' ? JSON.parse(p.items) : p.items;
      items.forEach((itemName) => {
        const item = expense.items.find(
          (i) =>
        ...
        const shared = expense.participants.filter((pp) => {
          const ppItems =
            typeof pp.items === 'string' ? JSON.parse(pp.items) : pp.items;
          return ppItems.some(
            (ii) => ii.toLowerCase().trim() === itemName.toLowerCase().trim()
          );
        }).length;
        total += toNum(item.price) / (shared || 1);
      });
  ...
  const handleToggleContact = (contactId) => {
    setSelectedContactIds((prev) => {
      if (prev.includes(contactId)) {
        const nextCalc = { ...splitCalculations };
        delete nextCalc[contactId];
        setSplitCalculations(nextCalc);
        return prev.filter((id) => id !== contactId);
      } else {
        const contact = relevantContacts.find((c) => c.id === contactId);
        const calc = calculateSplitsFromParticipants([contact]);
        setSplitCalculations((prevCalc) => ({ ...prevCalc, ...calc }));
  ...
  const selectedContacts = relevantContacts.filter((c) =>
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-xl shadow-xl w-[90%] max-w-2xl max-h-[90vh] flex flex-col"
      ...
      >
        ...
        <div className="flex items-start justify-between px-6 py-4 border-b border-gray-200">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              {expense.store_name} - ${fmt(expense.total_amount)}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        {error && (
          <div className="mx-6 mt-4 px-4 py-2 bg-red-50 border border-red-200 text-red-600 text-sm rounded-lg">
            {error}
          </div>
        )}
                ...
                {relevantContacts.length === 0 ? (
                  <div className="text-center py-8 text-gray-400">
                    ...
                    <p className="text-xs mt-1">
                      Make sure participant names match your contacts
                    </p>
                  </div>
                ) : (
                            ...
                            <div>
                              ...
                              <div className="space-y-2">
                                {calc.aaDetails?.map((aa, idx) => (
                                  <div
                                    key={idx}
                                    className="flex justify-between items-center px-3 py-2 bg-white border border-gray-200 rounded-md"
                                  >
                                      ...
                                      {aa.sharedWith > 1 ? (
                                        <>
                                          <div className="text-sm font-semibold text-blue-600">
                                            ${fmt(aa.amountPerPerson)}
                                          </div>
                                          <div className="text-xs text-gray-500">
                                            shared with {aa.sharedWith}
                                          </div>
                                        </>
                                      ) : (
        ...
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-gray-200">
          ...
          >
            ...
            {sending
              ? 'Sending...'
              : `Send Bills to ${selectedContactIds.length} ${
